package vivacurrency.io.vivawallet.activities;
...
import vivacurrency.io.vivawallet.data.WatchWallet;
import vivacurrency.io.vivawallet.fragments.FragmentPrice;
import vivacurrency.io.vivawallet.fragments.FragmentTransactionsAll;
import vivacurrency.io.vivawallet.fragments.FragmentWallets;
import vivacurrency.io.vivawallet.interfaces.NetworkUpdateListener;
import vivacurrency.io.vivawallet.services.NotificationLauncher;
import vivacurrency.io.vivawallet.services.WalletGenService;
import vivacurrency.io.vivawallet.utils.AddressNameConverter;
import vivacurrency.io.vivawallet.utils.Dialogs;
import vivacurrency.io.vivawallet.utils.ExchangeCalculator;
import vivacurrency.io.vivawallet.utils.ExternalStorageHandler;
import vivacurrency.io.vivawallet.utils.OwnWalletUtils;
import vivacurrency.io.vivawallet.utils.Settings;
import vivacurrency.io.vivawallet.utils.WalletStorage;
...
public class MainActivity extends SecureAppCompatActivity implements NetworkUpdateListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (preferences.getLong("APP_INSTALLED", 0) == 0) {
            Intent intro = new Intent(this, vivacurrency.io.vivawallet.activities.AppIntroActivity.class);
            startActivityForResult(intro, vivacurrency.io.vivawallet.activities.AppIntroActivity.REQUEST_CODE);
        }
    ...
    public void onActivityResult(int requestCode, int resultCode, final Intent data) {
        ...
        if (requestCode == vivacurrency.io.vivawallet.activities.QRScanActivity.REQUEST_CODE) {
            if (resultCode == RESULT_OK) {
                byte type = data.getByteExtra("TYPE", vivacurrency.io.vivawallet.activities.QRScanActivity.SCAN_ONLY);
                if (type == vivacurrency.io.vivawallet.activities.QRScanActivity.SCAN_ONLY) {
                    if (data.getStringExtra("ADDRESS").length() != 42 || !data.getStringExtra("ADDRESS").startsWith("0x")) {
                        snackError("Invalid Ethereum address!");
                        return;
                    }
                    Intent watch = new Intent(this, vivacurrency.io.vivawallet.activities.AddressDetailActivity.class);
                    watch.putExtra("ADDRESS", data.getStringExtra("ADDRESS"));
                    startActivity(watch);
                } else if (type == vivacurrency.io.vivawallet.activities.QRScanActivity.ADD_TO_WALLETS) {
                    if (data.getStringExtra("ADDRESS").length() != 42 || !data.getStringExtra("ADDRESS").startsWith("0x")) {
                        snackError("Invalid Ethereum address!");
                        return;
                    }
                ...
                } else if (type == vivacurrency.io.vivawallet.activities.QRScanActivity.REQUEST_PAYMENT) {
                    if (WalletStorage.getInstance(this).getFullOnly().size() == 0) {
                        Dialogs.noFullWallet(this);
                    } else {
                        Intent watch = new Intent(this, vivacurrency.io.vivawallet.activities.SendActivity.class);
                        watch.putExtra("TO_ADDRESS", data.getStringExtra("ADDRESS"));
                        watch.putExtra("AMOUNT", data.getStringExtra("AMOUNT"));
                        startActivity(watch);
                    }
                } else if (type == vivacurrency.io.vivawallet.activities.QRScanActivity.PRIVATE_KEY) {
        ...
        } else if (requestCode == vivacurrency.io.vivawallet.activities.WalletGenActivity.REQUEST_CODE) {
            if (resultCode == RESULT_OK) {
        ...
        } else if (requestCode == vivacurrency.io.vivawallet.activities.SendActivity.REQUEST_CODE) {
            if (resultCode == RESULT_OK) {
        ...
        } else if (requestCode == vivacurrency.io.vivawallet.activities.AppIntroActivity.REQUEST_CODE) {
            if (resultCode != RESULT_OK) {
                finish();
            } else {
                SharedPreferences.Editor editor = preferences.edit();
        ...
        } else if (requestCode == vivacurrency.io.vivawallet.activities.SettingsActivity.REQUEST_CODE) {
    ...
    public void importPrivateKey(String privatekey) {
        Intent genI = new Intent(this, vivacurrency.io.vivawallet.activities.WalletGenActivity.class);
        genI.putExtra("PRIVATE_KEY", privatekey);
        startActivityForResult(genI, vivacurrency.io.vivawallet.activities.WalletGenActivity.REQUEST_CODE);
    }
    ...
    private void selectItem(int position) {
        switch (position) {
            ...
            case 2: {
                Intent settings = new Intent(this, vivacurrency.io.vivawallet.activities.SettingsActivity.class);
                startActivityForResult(settings, vivacurrency.io.vivawallet.activities.SettingsActivity.REQUEST_CODE);
                break;
            }
            case 3: {
                new AlertDialog.Builder(this)
                        .setTitle("About VIVA")
                        .setMessage("VIVA is published under GPL3\n" +
                                "www.vivacurrency.io\n" +
                                getString(R.string.translator_name) + "\n" +
                                "\nCredits:\n" +
                                "Forked from the Lunary Simple Ether Wallet\n"+
                                ...
                                "VIVA is published under GPL3\n" +
                                "This app is not associated with Ethereum or the Ethereum Foundation in any way. VIVA is an independend wallet app.")
            ...
            case 4: {
                ...
                i.setData(Uri.parse("https://www.reddit.io/r/lunary"));
                startActivity(i);
                break;
            }
            case 5: {
