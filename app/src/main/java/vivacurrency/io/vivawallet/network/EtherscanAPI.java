package vivacurrency.io.vivawallet.network;
...
import vivacurrency.io.vivawallet.APIKey;
import vivacurrency.io.vivawallet.interfaces.LastIconLoaded;
import vivacurrency.io.vivawallet.interfaces.StorableWallet;
import vivacurrency.io.vivawallet.utils.Key;
import vivacurrency.io.vivawallet.utils.RequestCache;
import vivacurrency.io.vivawallet.utils.TokenIconCache;
...
public class EtherscanAPI {
    ...
    public void getPriceChart(long starttime, int period, boolean usd, Callback b) throws IOException {
        get("http://poloniex.io/public?command=returnChartData&currencyPair=" + (usd ? "USDT_ETH" : "BTC_ETH") + "&start=" + starttime + "&end=9999999999&period=" + period, b);
    }


    /**
     * Retrieve all internal transactions from address like contract calls, for normal transactions @see vivacurrency.io.simpleetherwallet.network.EtherscanAPI#getNormalTransactions() )
     *
     * @param address Ether address
     * @param b       Network callback to @see vivacurrency.io.simpleetherwallet.fragments.FragmentTransactions#update() or @see vivacurrency.io.simpleetherwallet.fragments.FragmentTransactionsAll#update()
    ...
    /**
     * Retrieve all normal ether transactions from address (excluding contract calls etc, @see vivacurrency.io.simpleetherwallet.network.EtherscanAPI#getInternalTransactions() )
     *
     * @param address Ether address
     * @param b       Network callback to @see vivacurrency.io.simpleetherwallet.fragments.FragmentTransactions#update() or @see vivacurrency.io.simpleetherwallet.fragments.FragmentTransactionsAll#update()
     * @param force   Whether to force (true) a network call or use cache (false). Only true if user uses swiperefreshlayout
     ...
     *
     * @param address Ether address
     * @param b       Network callback to @see vivacurrency.io.simpleetherwallet.fragments.FragmentDetailOverview#update()
     * @param force   Whether to force (true) a network call or use cache (false). Only true if user uses swiperefreshlayout
     * @throws IOException Network exceptions
     */
     ...
     * @param callback  Callback to @see vivacurrency.io.simpleetherwallet.fragments.FragmentDetailOverview#onLastIconDownloaded()
     * @throws IOException Network exceptions
     */
